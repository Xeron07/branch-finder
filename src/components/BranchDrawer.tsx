    ...
    <>
      ...
      >
        ...
        <div className="flex items-center justify-between px-5 py-3 border-b border-cream">
          <h2 className="font-playfair text-lg font-semibold text-midnight">Branch Details</h2>
          <button
            onClick={onClose}
        ...
        <div className="px-5 py-4 overflow-y-auto" style={{ maxHeight: "calc(85vh - 60px)" }}>
          {/* Branch name */}
          <h3 className="font-playfair text-xl font-bold text-midnight mb-1">{branch.name}</h3>
          ...
          <div className="space-y-3 mb-5">
            {branch.phone && (
              <div className="flex items-center gap-3">
                <div className="w-9 h-9 rounded-full bg-cream flex items-center justify-center shrink-0">
                  <svg className="w-4 h-4 text-sage" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.8}>
                    <path d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z" strokeLinecap="round" strokeLinejoin="round" />
                  </svg>
                </div>
                <a href={`tel:${branch.phone}`} className="text-[13px] text-midnight font-normal hover:text-gold transition-colors">
                  {branch.phone}
                </a>
              </div>
            )}
            ...
            {branch.email && (
              <div className="flex items-center gap-3">
                ...
                <a href={`mailto:${branch.email}`} className="text-[13px] text-midnight font-normal hover:text-gold transition-colors">
                  {branch.email}
                </a>
              </div>
            )}
